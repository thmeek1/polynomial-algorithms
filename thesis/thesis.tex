...
\section{Categories}
Over the last seventy-five years, the landscape of mathematics research has shifted in many ways. One of these ways is the ubiquity of categories in many mathematical fields. Modern mathematicians dealing with schemes, sheaves, or tensors are more likely to use their categorical definitions than classical constructions. There are increasingly many attempts to introduce category theory to undergraduate mathematics students\cite{Aluffi2009}. Still, category theory has not shed its reputation for being overly abstract and useless outside of the purest of mathematical endeavors. To be fair, the first criticism is justified. Category theory is indeed abstract. Perhaps this is why the aforementioned attempts to introduce this theory to a less experienced audience have not yet become standard practice. The second criticism however is simply unfounded. \par 
...
\begin{definition}
  ...
  \begin{itemize}
    ...
    \item If $f \in \Hom(a,b)$ and $g \in \Hom(b,c)$, then there is a morphism $gf \in \Hom(a,c)$. In this case, we say $f$ and $g$ are \textbf{composable} and $gf$ is a \textbf{composite}. 
  \end{itemize}
...
\begin{definition}
  ...
  \end{itemize}
  We call a functor $\cF: \sC \to \sC$ between a category and itself an \textbf{endofunctor}. The category of endofunctors on $\sC$ is denoted $\sC^\sC$. 
\end{definition}
\begin{definition}
...
The idea of commuting diagrams is central to the study of category theory. It is a concise way of saying that any allowed composition of maps in the diagram will be equivalent if they share a domain and a codomain. 
...
% \lipsum[76-85]
Suppose $n$ is given and let $\bF[\mathbf x] = \bF[x_1,\dots,x_n]$. Denote the set of finite ordered lists of polynomials in $\bF[\mathbf x]$ as $[\bF[\mathbf x]]$\footnote{This notation is meant to resemble the syntax for a list in Haskell.}. If the list $G$ is in $[\bF[\mathbf x]]$, then denote the $n$th entry in $G$ as $g_n$ and let $G \setminus g_n$ refer to the list obtained by removing $g_n$ from $G$ and maintaining the relative order of the rest of $G$. Let $\emptyset \in [\bF[\mathbf x]]$ denote the empty list. Let $\p: [\bF[\mathbf x]] \times \left( \bF[\mathbf x] \times \bF[\mathbf x] \right) \to \bF[\mathbf x] \times \bF[\mathbf x]$ be the function defined as 
