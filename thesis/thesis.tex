...
% A relationship that always holds between bind and \lstinline{join} is 
% \begin{lstlisting}[language=Haskell]
% x >>= f = join (fmap f x)
% \end{lstlisting}
% which can be quite useful when deciding how best to implement bind on a \lstinline{Monad} instance in a case where \lstinline{join} is easier to define.
