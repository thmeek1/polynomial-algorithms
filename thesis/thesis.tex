...
\end{definition}
The Lexicographic order is the order used in a dictionary. We begin with an ordering of the variables. The greater monomial is the one with the larger exponent in the first variable. If those exponents are the same, we instead calculate the Lexicographic order without the highest variable. In our previous example, $g = xy^2z^3 + x^5 + x^3y^2z$, we see that the lead term under the Lexicographic order where $x>y>z$ is $\LT(g)=x^5$. By convention, when denoting variables with subscripts, we take $x_1 > x_2 > x_3 > \dots$. Our next monomial ordering is built out of the Lexicographic order. 
...
\end{definition}
This means that the Graded Lex order first orders by total degree, but breaks ties with the Lexicographic order. Our third and final monomial ordering is less intuitive than the previous two.
...
\end{definition}
Here, we are still ordering first by total degree, but we break ties in a manner that is in some way the reverse of Lexicographic order. As unintuitive as the Graded Reverse Lex order may seem, it turns out that this ordering is often the most efficient in many algorithms, including some we will explore here. 
